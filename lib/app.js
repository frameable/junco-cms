...
var errors = require('express-async-errors')
...
module.exports.initialize = function (config) {
  ...
  var _gravatar_url = gravatar.url

  gravatar.url = function(email, options={}, protocol) {
    options.d = '404';
    return _gravatar_url(email, options, protocol)
  }
  ...
  app.use(function (req, res, next) {
    res.locals = {
      ...
      get proxyPath () {
        return config.application.proxyPath
      },
      get jingoVersion () {
  ...
  app.post('/uploadFile', upload.single('wiki_file'), function (req, res, next) {
    ...
    var repFile = app.locals.config.get("application").repository + '/uploads/' + fileName;
    ...
    fs.readFile(tempFile, function(err, data) {
      fs.writeFile(repFile, data, function(err) {
        fs.unlink(tempFile, async function() {
          var gitMsg = req.body.file_message;
          await Git.add(repFile, gitMsg, req.user.asGitAuthor);
          outputMsg = 'Reference uploaded media as<br><code>![](/uploads/' + fileName + ')</code>';
