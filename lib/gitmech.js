...
var gitMech = {
  ...
  logBulk: function (pattern, callback) {
    gitSpawn(['log', '--format=@@METADATA@@%x09%h%x09%H%x09%an%x09%ae%x09%aD%x09%ar%x09%at%x09%s', '--name-only'], function (err, data) {
  ...
  diff: function (path, revisions, callback) {
    ...
    gitSpawn([ ...colorOpts, 'diff', '--color-words=.', '-b', revisions, '--', docSubdir + path ], function (err, data) {
      callback(err, typeof data !== 'undefined' ? ansi.toHtml(data.toString()) : '')
    })
  },
